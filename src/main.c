...
// minter_main receives an APDU requesting that command, and is responsible for
...
//
// So a typical command flow looks something like this. We start in minter_main,
...
// IO_ASYNC_REPLY flag before returning. Control returns to minter_main, which
...
void io_exchange_with_code(uint16_t code, uint16_t tx) {
    G_io_apdu_buffer[tx++] = code >> 8u;
    G_io_apdu_buffer[tx++] = code & 0xFFu;
...
// in minter_main. It's common to set *flags |= IO_ASYNC_REPLY, but tx is
...
static void log_error(exception_t e) {
    PRINTF("Error occurred: [0x%04x] ", e);
    switch (e) {
        case INVALID_PARAMETER:PRINTF("Invalid parameter\n");
            break;
        case EXCEPTION_OVERFLOW:PRINTF("Stack overflow\n");
            break;
        case EXCEPTION_SECURITY:PRINTF("Security error\n");
            break;
        case INVALID_CRC:PRINTF("Invalid CRC\n");
            break;
        case INVALID_CHECKSUM:PRINTF("Invalid Checksum\n");
            break;
        case INVALID_COUNTER:PRINTF("Invalid Counter\n");
            break;
        case NOT_SUPPORTED:PRINTF("Unsupported\n");
            break;
        case INVALID_STATE:PRINTF("Invalid state\n");
            break;
        case TIMEOUT:PRINTF("Timeout error\n");
            break;
        case EXCEPTION_PIC:PRINTF("Timeout error\n");
            break;
        case EXCEPTION_APPEXIT:PRINTF("App exit error\n");
            break;
        case EXCEPTION_IO_OVERFLOW:PRINTF("IO overflow\n");
            break;
        case EXCEPTION_IO_HEADER:PRINTF("IO header\n");
            break;
        case EXCEPTION_IO_STATE:PRINTF("IO state\n");
            break;
        case EXCEPTION_IO_RESET:PRINTF("IO reset\n");
            break;
        case EXCEPTION_CXPORT:PRINTF("CX port\n");
            break;
        case EXCEPTION_SYSTEM:PRINTF("System error\n");
            break;
        case NOT_ENOUGH_SPACE:PRINTF("Not enough space\n");
            break;
...
static void minter_main(void) {
    ...
    for (;;) {
        ...
        //
        // In minter_main, this TRY block serves to catch any thrown exceptions
...
unsigned char io_event(unsigned char channel) {
    ...
    switch (G_io_seproxyhal_spi_buffer[0]) {
        case SEPROXYHAL_TAG_FINGER_EVENT: UX_FINGER_EVENT(G_io_seproxyhal_spi_buffer);
            break;

        case SEPROXYHAL_TAG_BUTTON_PUSH_EVENT: UX_BUTTON_PUSH_EVENT(G_io_seproxyhal_spi_buffer);
            break;
        ...
        case SEPROXYHAL_TAG_DISPLAY_PROCESSED_EVENT: UX_DISPLAYED_EVENT({ });
            break;

        case SEPROXYHAL_TAG_TICKER_EVENT: UX_TICKER_EVENT(G_io_seproxyhal_spi_buffer, { });
            break;
