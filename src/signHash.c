...
static const bagl_element_t ui_signHash_approve[] = {
    ...
    UI_TEXT(0x00, 0, 26, 128, global.signHashContext.deriveIndexStr),
};
...
static const bagl_element_t ui_signHash_compare[] = {
    ...
    UI_TEXT(0x00, 0, 26, 128, global.signHashContext.hashStrPart),
};
...
static const bagl_element_t *ui_prepro_signHash_compare(const bagl_element_t *element) {
    switch (element->component.userid) {
        ...
        case 2:
            ...
            return (ctx->displayIndex == sizeof(ctx->hashStrFull) - 12) ? NULL : element;
        default:
...
static unsigned int ui_signHash_compare_button(unsigned int button_mask, unsigned int button_mask_counter) {
    switch (button_mask) {
        ...
        case BUTTON_EVT_FAST | BUTTON_LEFT: // SEEK LEFT
            slideText(false, &ctx->displayIndex, ctx->hashStrFull, 64, ctx->hashStrPart);
            break;
        case BUTTON_RIGHT:
        case BUTTON_EVT_FAST | BUTTON_RIGHT: // SEEK RIGHT
            slideText(true, &ctx->displayIndex, ctx->hashStrFull, 64, ctx->hashStrPart);
            break;
        case BUTTON_EVT_RELEASED | BUTTON_LEFT | BUTTON_RIGHT: // PROCEED
            ...
            os_memmove(ctx->deriveIndexStr, "with Key #", 10);
...
void handleSignHash(uint8_t p1,
    ...
    bin2hex(ctx->hashStrFull, ctx->hash, sizeof(ctx->hash));
    os_memmove(ctx->hashStrPart, ctx->hashStrFull, 12);
    ctx->hashStrPart[12] = '\0';
    ctx->displayIndex = 0;
